...
namespace Hermes.ViewModels
{
	...
	{
		private CompetenceValidation CompetenceValidation;
		...
		public async Task AddNew()
		{
			try
			{
				...
				if (!result.Cancelled)
				{
					var newCompt = ((CompetenceValidation)result.Data).ToCompetence();
		...
		public async Task Edit(uint id)
		{
			...
			CompetenceValidation cptToEdit = new CompetenceValidation()
			{
			...
			if (!result.Cancelled)
			{
				var resultValidation = (CompetenceValidation)result.Data;
		...
		{
			CompetenceValidation = new CompetenceValidation();
			EditContextValidation = new EditContext(CompetenceValidation);
		}

		private Task<DialogResult> OpenDialog(string titre, CompetenceValidation modelValidation)
		{
